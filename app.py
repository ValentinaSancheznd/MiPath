...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    # Summary cards below use total arts jobs (avg_arts_jobs). A per-capita
    # ("jobs per 1,000 people") view was considered but dropped — it would
    # need state population, which isn't in the current pipeline's exports.
    metric_col = "avg_arts_jobs"
    metric_label = "Arts jobs"

    # ── Summary cards ────────────────────────────────────────────────────
    card_cols = st.columns(4)
    national_avg = state_summary[metric_col].mean()
    top_state_row = state_summary.sort_values(metric_col, ascending=False).iloc[0]

    if home_state_abbr:
        home_row_card = state_summary[state_summary["STABBR"] == home_state_abbr]
        if not home_row_card.empty:
            home_val = home_row_card.iloc[0][metric_col]
            home_rank = int(state_summary[metric_col].rank(ascending=False)[
                state_summary["STABBR"] == home_state_abbr
            ].iloc[0])
            card1_label = f"{metric_label} in {state_names.get(home_state_abbr)}"
            card1_val = f"{home_val:,.0f}"
            card3_label = f"{state_names.get(home_state_abbr)} rank"
            card3_val = f"{home_rank} of 50"
        else:
            card1_label, card1_val = metric_label, "N/A"
            card3_label, card3_val = "Rank", "N/A"
    else:
        card1_label, card1_val = "Pick a state", "—"
        card3_label, card3_val = "Rank", "Select a state"

    with card_cols[0]:
        st.markdown(f"<div class='cluster-card'><p style='font-size:11px;color:{MAROON};margin:0;'>{card1_label}</p>"
                     f"<p style='font-size:22px;color:{TERRACOTTA};margin:2px 0 0;'>{card1_val}</p></div>",
                     unsafe_allow_html=True)
    with card_cols[1]:
        st.markdown(f"<div class='cluster-card'><p style='font-size:11px;color:{MAROON};margin:0;'>National average</p>"
                     f"<p style='font-size:22px;color:{TERRACOTTA};margin:2px 0 0;'>{national_avg:,.0f}</p></div>",
                     unsafe_allow_html=True)
    with card_cols[2]:
        st.markdown(f"<div class='cluster-card'><p style='font-size:11px;color:{MAROON};margin:0;'>{card3_label}</p>"
                     f"<p style='font-size:22px;color:{TERRACOTTA};margin:2px 0 0;'>{card3_val}</p></div>",
                     unsafe_allow_html=True)
    with card_cols[3]:
        st.markdown(f"<div class='cluster-card'><p style='font-size:11px;color:{MAROON};margin:0;'>Top state</p>"
                     f"<p style='font-size:18px;color:{NEAR_BLACK};margin:2px 0 0;'>{top_state_row['state_full_name']}</p>"
                     f"<p style='font-size:12px;color:{MAROON};margin:0;'>{top_state_row[metric_col]:,.0f} {metric_label.lower()}</p></div>",
                     unsafe_allow_html=True)
    ...
    # Bar chart: states with the most opportunity, using the active toggle metric
    st.subheader(f"Top 10 states — {metric_label}")
    top10 = state_summary.dropna(subset=[metric_col]).sort_values(
        metric_col, ascending=False
    ).head(10).sort_values(metric_col)
    bar_colors = [TERRACOTTA if s == home_state_abbr else GREEN for s in top10["STABBR"]]
    fig_bar = go.Figure(go.Bar(
        x=top10[metric_col], y=top10["state_full_name"], orientation="h",
        marker_color=bar_colors,
        text=top10[metric_col].apply(lambda x: f"{x:,.1f}" if metric_col == "jobs_per_1000" else f"{x:,.0f}"),
        textposition="outside",
        hovertemplate="%{y}<extra></extra>"
    ))
    fig_bar = style_fig(fig_bar, height=360, showlegend=False)
    fig_bar.update_layout(yaxis_title="", xaxis_title=metric_label)
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if target_state:
        ...
        st.divider()
        st.subheader("Does it pay off over time?")
        traj_years = ["1 year", "4 years", "5 years"]
        traj_values = [row.get("median_earnings_1yr"), row.get("median_earnings_4yr"), row.get("median_earnings_5yr")]
        if any(pd.notna(v) for v in traj_values):
            fig_traj = go.Figure(go.Bar(
                x=traj_years, y=traj_values, marker_color=[TAN, GREEN, TERRACOTTA],
                text=[fmt_money(v) for v in traj_values], textposition="outside",
                hovertemplate="%{x}: $%{y:,.0f}<extra></extra>"
            ))
            fig_traj = style_fig(fig_traj, height=320, showlegend=False)
            fig_traj.update_layout(yaxis_title="Median earnings", xaxis_title="Time after graduation")
            st.plotly_chart(fig_traj, use_container_width=True)
            st.caption("Some points may be missing if Scorecard suppressed that year's cohort.")
