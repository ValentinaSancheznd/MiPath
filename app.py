...
for i, name in enumerate(tab_defs):
    with nav_cols[i]:
        ...
        if st.button(f"{i+1}. {name}", key=f"nav_{name}", width="stretch",
            ...
            # No st.rerun() here on purpose — clicking a widget already
            # triggers a natural rerun. Calling st.rerun() immediately cut
            # the script short before it ever reached the Field of study
            # widget further down, which is what was making Streamlit treat
            # that widget as "missing" and clear its saved value.
...
# ── TEMPORARY DEBUG — remove once the field-sync bug is confirmed fixed ────
st.caption(f"🔧 debug — session_state field: `{st.session_state.get('global_field_select')}` | "
            f"resolved selected_field: `{selected_field}` | active_tab: `{st.session_state.active_tab}`")
