...
def metric_card(label, value, sub_note=None):
    ...
    color = MAROON if value == "—" else TERRACOTTA
    ...
    st.markdown(
        ...
        f"<p style='font-size:24px;color:{color};margin:2px 0 0;'>{value}</p>{note_html}</div>",
        unsafe_allow_html=True
    )
...
# ═══════════════════════════════════════════════════════════════════════════
# FILTERS — all three always render on every tab (never conditionally
# removed), because Streamlit clears a widget's session_state if the code
# that creates it doesn't run in a given script pass. Preferred state and
# Employment year are disabled (greyed out, non-interactive) on tabs where
# they don't apply, instead of being hidden — this keeps their value intact
# across tab switches.
# ═══════════════════════════════════════════════════════════════════════════
...
state_disabled = st.session_state.active_tab not in ("Where to Go", "Cost & Payoff")
with st.sidebar.container(border=True):
    selected_state_name = st.selectbox("Preferred state", state_options,
                                         key="global_state_select", disabled=state_disabled)
    if state_disabled:
        st.caption("Not used on this tab")

year_disabled = st.session_state.active_tab != "Where to Go"
with st.sidebar.container(border=True):
    selected_year = st.select_slider("Employment year", options=year_slider_options,
                                       key="global_year_select", disabled=year_disabled)
    if year_disabled:
        st.caption("Not used on this tab")
...
if st.session_state.active_tab == "Explore Fields":
    ...
    bar_colors = [GREEN if f == selected_field else "rgba(107,158,78,0.28)" for f in field_sorted["CIPDESC"]]
    ...
    field_click = st.plotly_chart(fig_earnings, use_container_width=True,
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if spotlight_state:
        ...
        with col2:
            ...
            metric_card("Median earnings (4yr)", fmt_money(spotlight_row["median_earnings_4yr"]), note)
        with col3:
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if target_state:
        ...
        col1, col2, col3 = st.columns(3)
        with col1:
            metric_card("Net cost", fmt_money(row["median_net_cost"]))
        with col2:
            metric_card("Median debt", fmt_money(row["median_debt"]))
        with col3:
            metric_card("Earnings (4yr)", fmt_money(row["median_earnings_4yr"]))

        st.caption("**What this means:** earnings minus debt shows roughly how much of a graduate's "
                    "income is left over after covering typical student debt in the first few years "
                    "out of school — a rough signal of how quickly the degree starts to pay off.")
        ...
        if not nat_row.empty:
            ...
            metrics = ["Net Cost", "Median Debt", "Earnings (4yr)"]
            state_raw = [row["median_net_cost"], row["median_debt"], row["median_earnings_4yr"]]
            nat_raw = [nat_row["median_net_cost"], nat_row["median_debt"], nat_row["median_earnings_4yr"]]
