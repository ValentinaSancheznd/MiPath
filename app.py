...
# ═══════════════════════════════════════════════════════════════════════════
if st.session_state.active_tab == "Explore Fields":
    if "global_field_select" not in st.session_state:
        st.session_state["global_field_select"] = fields[0]
    selected_field = st.session_state["global_field_select"]

    st.subheader("Not sure yet? Start here.")
    st.caption("Click a bar to choose a field — comparing every arts field before narrowing in on one.")
    ...
    field_click = st.plotly_chart(fig_earnings, use_container_width=True,
                                    on_select="rerun", key="explore_field_click")
    ...
    if clicked_pts:
        ...
        if clicked_field and clicked_field != selected_field:
            st.session_state["global_field_select"] = clicked_field
            st.rerun()
    ...
    st.divider()
    st.subheader("The bigger picture: U.S. arts employment")
    trend_year_options = sorted(trend_national["year"].unique().tolist())
    highlight_year = st.select_slider("Highlight a year", options=trend_year_options,
                                        value=trend_year_options[-1])

    fig_trend = go.Figure(go.Scatter(
        x=trend_national["year"], y=trend_national["arts_employment"],
        mode="lines+markers", line=dict(color=GREEN, width=3),
        marker=dict(
            size=[14 if y == highlight_year else 8 for y in trend_national["year"]],
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if spotlight_state:
        ...
        st.markdown(
            ...
            f"<h3 style='margin:0;'>{spotlight_label}: {spotlight_row['state_full_name']}</h3>"
    ...
    with map_col:
        ...
        map_event = st.plotly_chart(fig_map, use_container_width=True,
                                      on_select="rerun", key="wtg_map_click")
        ...
        if clicked_points:
            ...
            if clicked_loc and clicked_loc in state_names:
                ...
                if st.session_state.get("global_state_select") != clicked_full_name:
                    st.session_state["global_state_select"] = clicked_full_name
                    st.rerun()
